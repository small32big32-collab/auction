...
def get_items_by_key(license_key: str):
    ...
    items = supabase.table("price_history").select("item_id, item_name, rarity, category").execute()
...
def update_sniper_threshold(sniper_id: int, data: SniperUpdate):
    try:
...
def delete_single_sniper(sniper_id: int):
    try:
