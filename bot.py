...
async def show_main_menu(message_or_callback, edit=False, has_key=False):
  ...
  text = (
      ...
      "Добро пожаловать! Инструмент для мониторинга и анализа цен аукциона артефактов[cite: 4].\n"
...
async def process_about_tariffs(callback: types.CallbackQuery, state: FSMContext):
  ...
  text = (
      ...
      "Лицензионный доступ на 30 дней[cite: 4]."
  )
  ...
  async with httpx.AsyncClient() as client:
    try:
      ...
      if res.status_code == 200:
        ...
        await callback.message.edit_text(f"📊 **{item_name}** ({selected_rarity})\nНажмите кнопку ниже для настройки снайпера.", reply_markup=builder.as_markup(), parse_mode="Markdown")
    except Exception as e:
      await callback.message.answer(f"Ошибка: {e}")
  await callback.answer()
...
async def process_sniper_threshold_input(message: types.Message, state: FSMContext):
  ...
  # Сохраняем снайпер в Supabase через API (или напрямую в таблицу user_snipers)
  # Пример отправки запроса на бэкенд для сохранения снайпера:
  async with httpx.AsyncClient() as client:
    try:
...
async def process_sniper_menu(callback: types.CallbackQuery, state: FSMContext):
  ...
  # Запрашиваем активные снайперы пользователя из базы
  async with httpx.AsyncClient() as client:
